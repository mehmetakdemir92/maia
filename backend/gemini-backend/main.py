...
# Comma-separated fallback lists (tried in order). The "-002" Vertex-only
# pinned snapshots are gone — the free Developer API only knows the bare
# model names, which also work fine on Vertex, so this list is compatible
# with both.
...
ALT_MODELS = _parse_model_list(
    ...
    [ALT_MODEL, "gemini-2.5-pro", "gemini-1.5-pro"],
)

# Free Gemini Developer API (ai.google.dev), not paid Vertex AI — no
# GCP billing needed for this call path. Swapped from `vertexai=True,
# project=..., location=...` because that route requires an active
# billing account on the GCP project even within otherwise-free quota.
client = genai.Client(api_key=os.environ["GEMINI_API_KEY"])

def _is_quota_exhausted(exc: Exception) -> bool:
    # google.genai.errors.ClientError on the free Developer API, not the
    # google.api_core exception classes the Vertex SDK used to raise — string
    # match is what actually catches it now.
    msg = str(exc).lower()
    ...
    for model_name in models:
        for attempt in range(2):
            try:
                ...
                last_err = exc
                print(
                    f"Gemini call failed [model={model_name}] "
                    f"[{type(exc).__name__}]: {exc}\n"
                    f"{traceback.format_exc()}"
                )
                # The free Developer API's per-minute quota is shared across
                # every model on this key, so once it's hit, cycling through
                # fallback models or retrying with backoff just burns more of
                # it for nothing — fail fast with a message the client already
                # knows how to show (ExampleGenerator.friendlyBackendError
                # maps 429 to "busy right now, wait a few seconds").
                if _is_quota_exhausted(exc):
                    raise HTTPException(
                        status_code=429,
                        detail="Quota exceeded for the free tier. Please wait a minute and try again.",
                    )
                if not _is_retryable(exc) or attempt >= 1:
                    break
                sleep_s = min(2.0, 0.5 * (2**attempt))
                time.sleep(sleep_s)
        # try next model
        continue
