...
/// Manages word progress storage and retrieval
class WordProgressManager: ObservableObject {
    ...
    private let progressKey = "wordProgressMap"
    ...
    private func saveProgress() {
        // Convert [UUID: WordProgress] to [String: WordProgress] for encoding
        let stringDict = Dictionary(uniqueKeysWithValues: progressMap.map { (key, value) in
            (key.uuidString, value)
        })
        
        guard let encoded = try? JSONEncoder().encode(stringDict) else {
            return
        }
